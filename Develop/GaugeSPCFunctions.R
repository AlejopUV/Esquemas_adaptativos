...
conv<-function(delta,r,q,mu=10,sig=1,dist="norm",skew=0, f=0.5){ #mu y sig en escala original
  ...
  if(dist=="norm"){
    ...
    lambda=d/sqrt(1-d^2)
    zs=qsn(f*q,0,1,lambda)
    zl=qsn(1-(1-f)*q,0,1,lambda)
...
ref.weibull<-function(eta,a,b){
  gam1=gamma(1+(1/b));gam2=gamma(1+(2/b));gam3=gamma(1+(3/b))
  mean<-eta+a*gam1
  sd<-a*sqrt(gam2-(gam1^2))
...
ref.lognorm3<-function(eta,mulog,siglog){
  mean<-round(eta+exp(mulog+0.5*(siglog^2)),2)
...
pwYsYl<-function(w,n,qs,ql){
  ...
  if ((min(qs,ql) < 0.0)|((qs + ql) > 1.0)| (n <= 0)) {stop("Parametros Inconsistentes")}
    ...
    wYsYl=Prob=0
    i=1; fact.n<-factorial(n)
    for (s in 0:n){
       cte=fact.n/factorial(s)
      for (l in 0:(n-s)){
        wYsYl[i]<-max(w*s+l,s+w*l)
        Prob[i] = (cte/(factorial(l)*factorial(n-s-l)))*(qs^s)*(ql^l)*((1-qs-ql)^(n-s-l))
        i=i+1
      }
    }
  }
...
CARLwYsYl<-function(n,Lc,w,q,delta,r,curve="mean",mu=10,sig=1,dist="norm",skew=0,f=0.5){ 
  ARL=0

  if(curve=="dev"){
    for(i in 1:length(r)){
      qd<-conv(0,r[i],q,mu,sig,dist,skew,f)[1:2]
    ...
    for(i in 1:length(delta)){
      qd<-conv(delta[i],1,q,mu,sig,dist,skew,f)[1:2]
      ARL[i]<-ARLwYsYl(n,Lc,w,qd[1],qd[2])
    }
    return(cbind(delta,ARL))
  }
...
#ARL wYsYl-EWMA con numero fijo de estados (m)
...
ARLwYsYl.EWMA<-function(n,Lim,w,lambda,q,delta,r,opt=1,mu=10,sig=1,dist="norm",skew=0,amp=0.01,image=F){
          ...
          #generar tabla con distribucion de wYSYL
          if((lambda > 1.0)| (lambda<=0)| (Lim<=0)) {cat(c(lambda,Lim));stop("Parametros Inconsistentes")}
...
RootARL.wYsYl<-function(n,Lc,w,ARLdes,f=0.5){
   Tol=0.001
   ninc<-0
   fact.n<-factorial(n)
      ...
      for(s in 0:n){
         ...
         for(l in 0:(n-s)){
            if((w*s+l>=Lc)|(s+w*l>=Lc)){
               ...
               sumdf<-sumdf + cte*((f)^s)*((1-f)^l)*((qi)^(s+l-1))*((1-qi)^(n-s-l-1))*(s+l-n*qi)
               Prob = Prob + fact.n*cte*(qi^(s+l))*(f^s)*((1-f)^l)*((1-qi)^(n-s-l))}
         }
      }
      fi<- Prob*ARLdes -1
      dfi<- ARLdes*fact.n*sumdf
      q<- qi-fi/dfi
...
Genera.LC<-function(w,n,decreasing = FALSE){
   ...
   LC<-LC[LC>0]
   return(sort(LC,decreasing = FALSE))
}

#Optimizer of wYsYl and specific schemes (esq="wYsYl","YSYL","YT","YS-YL") for a shift (delta,r) in the process

Opt.YsYl<-function(n,delta,r,ARLdes,esq=c("wYsYl","YsYl","Ys-Yl","YT"),mu=10,sig=1,dist="norm",skew=0,search.f=F,UB.restr=F,ESE=T){ 
   
   esq.val=c("Ys-Yl","YsYl","YT","wYsYl")
   if(is.na(match(esq,esq.val))){stop("esq must be: wYsYl, YsYl, Ys-Yl or YT")}
   
   t <- proc.time()
   par<-c(n,delta,r,ARLdes)
   names(par)<-c("n","delta","r","ARL.0 deseado")
  
   cat("\n","_ _ _ _ _ Input _ _ _ _ _ ","\n")
   print(par)
   cat("\n","Esquema: ", esq)
   cat("\n","Distribuci?n: ", dist, ", Asimetria: ", skew)
  dw=0.1; w.min=-2; df=0.025;e=0.02; cont=0;
   
   if(esq=="wYsYl"){w.p=c(1,-1,0,seq(w.min,-1-dw,dw),seq(-1+dw,-dw,dw),seq(dw,(1-dw),dw))}else{w.p = match(esq,esq.val)-2}
   if(search.f&(dist!="norm")){f.p=c(0.5,seq(0.1,0.5-df,df),seq(0.5+df,0.9,df))
   f.p=f.p[order(abs(f.p-0.5))]
   }else{f.p=0.5}
   cond=F
   optimos<-data.frame(n=n,Lc=0,w=0,q=0,f=0,ARL1=0,ARL0=0)
   
   for (w in w.p){
      L.p=Genera.LC(w,n)
      for(Lc in L.p){
         for(f in f.p){
            # se puede verificar a priori la codicion de q, para evitar que itere en NR
            q<-RootARL.wYsYl(n,Lc,w,ARLdes,f)
            ARL0=ARLwYsYl(n,Lc,w,q*f,q*(1-f))
            min.col<-min(f*q,(1-f)*q)#estaverificando a posterior...
            #if(q<=0|q>=1){print("Error q")}
            if(min.col<0.005|min.col>0.498|abs(ARL0-ARLdes)>0.8){next}
            else{
               
               if((dist=="norm"|(delta==0))){
                  
                  qd1<-conv(delta,r,q,mu,sig,dist,skew,f)[1:2]
                  ARL1=ARLwYsYl(n,Lc,w,qd1[1],qd1[2])
               }
               else{clas=T
                  if(UB.restr){ # verifica la condici?n y cambia el estado si no se cumple
                     qde1<-conv(e,1,q,mu,sig,dist,skew,f)[1:2]
                     qde2<-conv(-e,1,q,mu,sig,dist,skew,f)[1:2]
                     T1<-ARLwYsYl(n,Lc,w,qde1[1],qde1[2])
                     T2<-ARLwYsYl(n,Lc,w,qde2[1],qde2[2])
                     clas=ifelse(max(T1,T2)<ARLdes,T,F)}
               
                  if(clas==T){#Soluci?n que clasifica
                     qd1<-conv(delta,r,q,mu,sig,dist,skew,f)[1:2] #lo calcula innecesario, primero verificar ArL unbiased
                     A1=ARLwYsYl(n,Lc,w,qd1[1],qd1[2])
                     qd2<-conv(-delta,r,q,mu,sig,dist,skew,f)[1:2]
                     A2= ARLwYsYl(n,Lc,w,qd2[1],qd2[2])
                     ARL1=max(A1,A2)}else{next}
               }
               
            cont=cont+1
            optimos[cont,]<-c(n,Lc,w,q,f,ARL1,ARL0)
            }
         }
      }
   }

   # selecci?n de las dos soluciones
   
   optimos<-optimos[order(optimos$ARL1),]
   optimo.G=optimos[1,] 
    
    if(ESE==F){
      sol<-cbind(optimo.G,1)
    }
   else{
      opt.c=with(optimos,((ARL1<(optimo.G$ARL1*1.025))|((ARL1-optimo.G$ARL1)<0.1)))
      optimos.c=optimos[opt.c,]
      orden= with(optimos.c,order(ifelse(w==-2,3,ifelse(w==0,2,ifelse(w==-1,1,ifelse(w==1,0,4))))
                  ,abs(f-0.5)))
       optimos.c=optimos.c[orden,]
       sol=cbind(optimos.c[1,],round((optimos.c[1,6]/optimo.G$ARL1)-1,4))
    }

   
   sol=as.numeric(sol)
   
   Lc.opt=sol[2];w.opt=sol[3];q.opt=sol[4];f.opt=sol[5]
   
   t <- proc.time()-t
   cat("\n","_ _ _ _ _ Output _ _ _ _ _ ","\n")
   cat("\n","Tiempo de ejecucion: ", round(t[3],2), " Segundos")
   cat("\n","    ARL0 =", round(ARLwYsYl(n,Lc.opt,w.opt,q.opt*f.opt,q.opt*(1-f.opt)),4))
   if((dist=="norm")|(delta==0)){
      ARL1.p<-sol[6];ARL1.n<-NA
      cat("\n","    ARL1 =", round(ARL1.p,3))
   }else{
      qd1<-conv(delta,r,q.opt,mu,sig,dist,skew,f.opt)[1:2]
      qd2<-conv(-delta,r,q.opt,mu,sig,dist,skew,f.opt)[1:2]
      ARL1.p<-ARLwYsYl(n,Lc.opt,w.opt,qd1[1],qd1[2])
      ARL1.n<-ARLwYsYl(n,Lc.opt,w.opt,qd2[1],qd2[2])
      cat("\n","    ARL1(delta,r)=", round(ARL1.p,3))
      cat("\n","    ARL1(-delta,r)=", round(ARL1.n,3))
   }
   sol<-c(n,round(Lc.opt,2),round(w.opt,3),round(q.opt,5),round(f.opt,3),round(ARL1.p,3),round(ARL1.n,3),sol[8])
   names(sol)<-c("n","Lc","w","q","f","ARL1+","ARL1-","%opt")
   if(ESE==F){
      return(list(sol,optimo.G))
   }
   else{return(list(sol,optimos.c))}
...
GA.D2YsYl<-function(n,delta,r,ARL0obj,mu,sig,dist,skew,mobj=T){
  ...
  names(par)<-c("n","delta","r","ARL.0 deseado")
  q.min<-0.001;q.max<-0.65; prec.q<-0.0001
  prec.L<-0.01 
...
GA.wYsYlTMV<-function(n,delta,r,ARL0obj,n2.max=2*n,mu,sig,dist,skew,maxiter,F.Lc1=NA,F.w=NA,F.n1=NA,fit=2){
  ...
  prec.w<-0.05
  prec.q<-0.0001
  prec.L<-0.005 ###Fundamental para la precision del algoritmo.....(0.05, mejor opcion)
  ...
  GA.sol<-ga(type = "binary", nBits = sum(l), fitness = fitn.wYsYlTMV,l=l,maxim=maxim,F.Lc1=F.Lc1,F.w=F.w,n.max,F.n1,fit=fit,
             popSize = 500, maxiter = maxiter, run = 300, pcrossover = 0.95, pmutation= 0.20) #.pendiente modificar tipo d cruce y seleccion
...
GA.wYsYlEWMA<-function(n,delta,r,ARL0obj,mu,sig,dist,skew,maxiter,F.w=NA,fit=2){
          ...
          prec.w<-0.05 ; prec.q<-0.0001; prec.Lim<-0.1; prec.Lam<-0.01 
...
######nuevas funciones GA para wYsYl- DS-VSS-EWMA con precisi?n controlada en los parametros de dise?o

decode.wYsYlTMV <- function(string,l,maximos,minimos,prec,F.Lc1,F.w,F.n1) {#l:longitud de las cadenas,
   #string <- gray2binary(string) #convierte de codificaci0n de gray a binaria habitual
...
fitn.wYsYlTMV<-function(string,l,maximos,minimos,prec,F.Lc1,F.w,F.n1,fit,ARL.ref){
   sol<-decode.wYsYlTMV(string,l,maximos,minimos,prec,F.Lc1,F.w,F.n1)
   
   #validaci?n de la soluci?n
   sol.valid=ifelse((sum(sol>maximos)>0)|(sum(sol<minimos)>0)|(sol[5]>=sol[6]),F,T)
   
   if(sol.valid==F){fitn<-0}
   else{ q<-sol[1];w<-sol[2];n1<-sol[3];n2<-sol[4];Lw<-sol[5];Lc1<-sol[6];Lc2<-sol[7]
   ARL=ARLwYsYl.TMV(n1,n2,Lc1,Lw,Lc2,w,q,delta,r,mu,sig,dist,skew)
   if(ARL[1]==Inf|ARL[4]==Inf|ARL[1]<1|ARL[4]<1){fitn<-0}
   else{
      if(fit==1){ fitn<-10000-w1*((ARL[1]<ARL0obj)*(ARL0obj-ARL[1]))-w3*(ARL[2]>n)*(ARL[2]-n)-w2*(ARL[4]/ARL.ref)}
         ...
         fitn<-10000-w1*((ARL[1]<ARL0obj)*2*abs(ARL[1]-ARL0obj)+(ARL[1]>=ARL0obj)*abs(ARL[1]-ARL0obj))-w3*abs(ARL[2]-n)-w2*(ARL[4]/ARL.ref)}
   }}
   fitn
}

GA.wYsYlTMV<-function(n,delta,r,ARL0obj,n2.max=2*n,mu,sig,dist,skew,maxiter,F.Lc1=F,F.w=F,F.n1=F,fit=2){
   t <- proc.time() 
   par<-c(n,delta,r,ARL0obj)
   names(par)<-c("n","delta","r","ARL.0 deseado")
   
   prec.w<-0.1;prec.q<-0.0001;prec.L<-0.01 
   q.min<-0.001; q.max<-ifelse(delta==0,0.5,0.65)
   w.min<-(-2.0); w.max<-1.0
   
   maximos=c(q.max,w.max,n-1,n2.max,1.0,1+prec.L,1.0)
   minimos=c(q.min,w.min,1,n+1,prec.L,prec.L,prec.L)
   ...
   GA.sol<-ga(type = "binary", nBits = sum(l), fitness = fitn.wYsYlTMV,l=l,
              maximos=maximos,minimos=minimos,prec=prec,F.Lc1=F.Lc1,F.w=F.w,F.n1=F.n1,fit=fit,ARL.ref=ARL.ref,
              popSize = 500, maxiter = maxiter, run = 300, pcrossover = 0.95, pmutation= 0.20) #.pendiente modificar tipo d cruce y seleccion
   
   sol<-decode.wYsYlTMV(GA.sol@solution[1,],l,maximos,minimos,prec,F.Lc1,F.w,F.n1)
   q<-sol[1];w<-sol[2];n1<-sol[3];n2<-sol[4];Lw<-sol[5];Lc1<-sol[6];Lc2<-sol[7]
   sol.ARL=ARLwYsYl.TMV(n1,n2,Lc1,Lw,Lc2,w,q,delta,r,mu,sig,dist,skew)
   salida<-round(c(n1,n2,Lc1,Lw,Lc2,w,q,sol.ARL),4)
   t <- proc.time()-t
...
decode.wYsYlDS <- function(string,l,maximos, minimos,prec,F.Lc1,F.w,F.n1) {#l:longitud de las cadenas,
   #string <- gray2binary(string) #convierte de codificaci0n de gray a binaria habitual
   q = minimos[1]+ prec[1]*binary2decimal(string[1:l[1]])
   w =  ifelse(class(F.w)=="logical"&(F.w==F),minimos[2]+ prec[2]*binary2decimal(string[(l[1]+1):sum(l[1:2])]),F.w)
   n1 =  ifelse(F.n1==F,minimos[3]+ prec[3]*binary2decimal(string[(sum(l[1:2])+1):sum(l[1:3])]),F.n1)
...
fitn.wYsYlDS<-function(string,l,maximos,minimos,prec,F.Lc1,F.w,F.n1,fit,ARL.ref){
   sol<-decode.wYsYlDS(string,l,maximos,minimos,prec,F.Lc1,F.w,F.n1)
   q<-sol[1];w<-sol[2];n1<-sol[3];n2<-sol[4];Lw<-sol[5];Lc1<-min(sol[6],n1+0.1);Lc2<-sol[7]
...
GA.wYsYlDS<-function(n,delta,r,ARL0obj,n.max=2*n,mu,sig,dist,skew,maxiter,F.Lc1=F,F.w=F,F.n1=F,fit=2){
   t <- proc.time() 
   par<-c(n,delta,r,ARL0obj)
   names(par)<-c("n","delta","r","ARL.0 deseado")
...
decode.wYsYlEWMA <- function(string,l,minimos, maximos,prec,F.w){#l:longitud de las cadenas,
   #string <- gray2binary(string) #convierte de codificaci0n de gray a binaria habitual
   
   q = minimos[1]+ prec[1]*binary2decimal(string[1:l[1]])
   w =  ifelse(class(F.w)=="logical"&(F.w==F),minimos[2]+ prec[2]*binary2decimal(string[(l[1]+1):sum(l[1:2])]),F.w)
   Lam =  minimos[3]+ prec[3]*binary2decimal(string[(sum(l[1:2])+1):sum(l[1:3])])
   L =  minimos[4]+ prec[4]*binary2decimal(string[(sum(l[1:3])+1):sum(l[1:4])])
   
   return(c(q,w,Lam,L))
}

fitn.wYsYlEWMA<-function(string,maxim,l,minimos,maximos,prec,F.w,fit,ARL.ref){
   sol<-decode.wYsYlEWMA(string,l,minimos,maximos,prec,F.w)
   
   #validaci?n de la soluci?n
   sol.valid=ifelse((sum(sol>maximos)>0)|(sum(sol<minimos)>0),F,T)
   
   if(sol.valid==F){fitn<-0}else{
      
      q<-sol[1];w<-sol[2];Lam<-sol[3];L<-sol[4]
      ARL=ARLwYsYl.EWMA(n,L,w,Lam,q,delta,r,opt,mu,sig,dist,skew)
      if(ARL[3]==Inf|ARL[4]==Inf|ARL[3]<1|ARL[4]<1){fitn<-0}
      else{
         if(fit==1){fitn<-10000-w1*max(ARL0obj-ARL[3],0)-w2*(ARL[4]/ARL.ref)}
         else{fitn<-10000-w1*((ARL[3]<ARL0obj)*2*(ARL0obj-ARL[3])+(ARL[3]>=ARL0obj)*(ARL[3]-ARL0obj))-w2*(ARL[4]/ARL.ref)}
      }
   }
   fitn
}
...
GA.wYsYlEWMA<-function(n,delta,r,ARL0obj,mu,sig,dist,skew,maxiter,F.w=F,fit=2){
   t <- proc.time() 
   param<-c(n,delta,r,ARL0obj)
   names(param)<-c("n","delta","r","ARL.0 deseado")
   
   
   prec.w<-0.1;prec.q<-0.0001;prec.L<-0.05; prec.Lam<-0.05 
   q.min<-0.001; q.max<-ifelse(delta==0,0.5,0.65)
   w.min<-(-2.0); w.max<-1.0
   
   maximos=c(q.max,w.max,1,4)
   minimos=c(q.min,w.min,prec.Lam,prec.L)
   prec=c(prec.q,prec.w,prec.Lam,prec.L)
   
   l1=length(decimal2binary((q.max-q.min)/prec.q))#q
   l2=ifelse(class(F.w)=="logical"&(F.w==F),length(decimal2binary((w.max-w.min)/prec.w )),0)#w
   l3=length(decimal2binary((1-prec.Lam)/prec.Lam))#Lambda
   l4=length(decimal2binary((4-prec.L)/prec.L))#L
   l<-c(l1,l2,l3,l4)
   
   ARL.ref=ARLXS(n,1/ARL0obj,delta,r)
   
   GA.sol<-ga(type = "binary", nBits = sum(l), fitness = fitn.wYsYlEWMA,l=l,minimos=minimos, maximos=maximos,prec=prec,
              F.w=F.w,fit=fit,ARL.ref=ARL.ref,
              popSize = 300, maxiter = maxiter, run = 30, pcrossover =0.95 , pmutation= 0.25,parallel = F) #.pendiente modificar tipo d cruce y seleccion
   
   sol<-decode.wYsYlEWMA(GA.sol@solution[1,],l, minimos, maximos, prec,F.w)
   q<-sol[1];w<-sol[2];Lam<-sol[3];L<-sol[4]
   sol.ARL=ARLwYsYl.EWMA(n,L,w,Lam,q,delta,r,opt,mu,sig,dist,skew)
   salida<-c(n,L,w,Lam,q,sol.ARL[3:4])
   t <- proc.time()-t
   print(param)
   print("_________________________________")
   print(paste("Tiempo de ejecucion: ", round(t[3],2), " Segundos"))
...
#wYsYl Simulator 
SARLwYsYl<-function(n,Lc,w,q,delta,r,mu=10,sig=1,dist="norm",skew=0,f=0.5,it=10000){
